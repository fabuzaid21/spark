...
import org.apache.spark.ml.tree.impl.AltDT.{AltDTMetadata, FeatureVector}
...
//    val newInfo = info.update(bitVector, activeNodeMap, newNumNodeOffsets = 3, labels, metadata)
//
...
//
//    val newInfo2 = newInfo.update(bitVector2, activeNodeMap2, newNumNodeOffsets = 5, labels, metadata)
//
...
//    val activeNodes = new BitSet(1)
//    activeNodes.set(0)
//    val splits1 = AltDTClassification.chooseSplitsForActiveNodes(col1, activeNodes, Array(fromOffset, toOffset), Array(fullImpurityAgg), labelsAsBytes, metadata)
...
//    val splits2 = AltDTRegression.chooseSplitsForActiveNodes(col2, activeNodes, Array(fromOffset, toOffset), Array(fullImpurityAgg), labels, metadata)
...
//          labels, 0, values.length, fullImpurityAgg, metadata, featureArity)
//      split match {
...
//      labels, 0, values.length, fullImpurityAgg, metadata, featureArity, allSplits)
//    split match {
//      case Some(s: CategoricalSplit) =>
//        assert(s.featureIndex === featureIndex)
...
//
  ...
  test("chooseContinuousSplitSparse: basic case") {
    ...
    val splitsAndStats = AltDTClassification.chooseContinuousSplitsSparseForActiveNodes(featureIndex,
...
//    labels.foreach(label => fullImpurityAgg.update(label))
//
//    val (split, stats) = AltDTRegression.chooseContinuousSplit(featureIndex, values,
...
//    labels.foreach(label => fullImpurityAgg.update(label))
//
//    val (split, stats) = AltDTClassification.chooseContinuousSplit(featureIndex, values, values.indices.toArray,
