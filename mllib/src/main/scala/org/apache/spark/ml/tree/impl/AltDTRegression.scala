...
object AltDTRegression {
  ...
  def trainImpl(
    ...
    while (currentLevel < maxDepth && !doneLearning) {
      ...
      println(s"currentLevel: $currentLevel, bestSplitsAndGains: ${bestSplitsAndGains.mkString(", ")}")
      ...
      println(s"activeNodeMap: $activeNodeMap")
      ...
      if (!doneLearning) {
        ...
        println(s"bitvector: $aggBitVector")
        ...
        println("printing PartitionInfos")
        partitionInfos.foreach(println(_))
  ...
  private[impl] def computeBestSplits(
    ...
    val partBestSplitsAndGains: RDD[Array[(Option[Split], ImpurityStats)]] = partitionInfos.map {
      ...
      activeNodes: BitSet, fullImpurityAggs: Array[ImpurityAggregatorSingle]) =>
        ...
        val toReturn = chooseSplitsForActiveNodes(columns(0), activeNodes, nodeOffsets, fullImpurityAggs, localLabels, metadata)
  ...
  def chooseContinuousSplitsSparseForActiveNodes(
    ...
    println(s"fullImpurities: ${fullImpurities.mkString(", ")}")
